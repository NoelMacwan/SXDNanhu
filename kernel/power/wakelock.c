...
enum {
	...
	DEBUG_PMS_WAKE_LOCK = 1U << 6,  //Tamsui3-kernel-BH-PMSWakelockInfo-00+
};
...
//Tamsui3-kernel-BH-PMSWakelockInfo-00+[
#ifdef CONFIG_TAMSUI_DUMP_WAKELOCK
...
void add_pms_wakelock_info(char *pid, char *tag, char * cmdline) 
{
	...
	list_for_each_entry(lock, &pms_locks, link) {
		if (!strcmp(lock->tag,tag) && !strcmp(lock->pid,pid))
		{
			...
			if (debug_mask & (DEBUG_PMS_WAKE_LOCK)) //Tamsui3-kernel-BH-PMSWakelockInfo-01+
...
#endif /* CONFIG_TAMSUI_DUMP_WAKELOCK */
//Tamsui3-kernel-BH-PMSWakelockInfo-00+]
