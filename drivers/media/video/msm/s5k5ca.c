...
static int32_t s5k5ca_set_brightness(struct exp_gain_cfg	*bright)
{
	...
	if(s5k5ca_brightness[j]!=NULL)
	{
        ...
        int8_t j=0;
        j = (bright->gain);
        rc=s5k5ca_i2c_write_b(s5k5ca_client->addr,0x0028,0x7000);
        if(rc<0) goto set_brightness_faill;
	rc=s5k5ca_i2c_write_b(s5k5ca_client->addr,0x002a,0x020C);
        if(rc<0) goto set_brightness_faill;
        rc=s5k5ca_i2c_write_b(s5k5ca_client->addr,0x0F12,j*10);
        if(rc<0) goto set_brightness_faill;     
set_brightness_faill:
	pr_emerg("Peter-s5k5ca_set_brightness_faill() rc:%x ",rc);
