...
static char tianma_color2_control[65] = {0xE3, 0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,
	0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0};
#endif
static char tianma_command1[5] = {0xF7, 0xA9,0x91,0x2D,0x8A};
...
static char turly_power_control3[3] = {0xC5,0x54, 0x54};
...
static char turly_display_function_control[3] = {0xB6, 0x02,0x02}; //{0xB6, 0x20,0x22};
...
static int mipi_ili9486_reg_control(int on)
{
	...
	if(vreg_state == on  ) 
	{
		printk(KERN_NOTICE "we do not update vreg \n");
		return 0;
	}
	...
	else if(has_ignore_vreg_set && !on)
	{
...
if(LCM_ID==1)
{
	mipi_dsi_cmds_tx(mfd, &ili9486_tx_buf, ili9486_tianma_display_on_cmds,
			ARRAY_SIZE(ili9486_tianma_display_on_cmds));
}	
else
{
mipi_dsi_cmds_tx(mfd, &ili9486_tx_buf, ili9486_turly_display_on_cmds,
...
static int mipi_ili9486_lcd_off(struct platform_device *pdev)
{
	...
	//[Arima Edison] add to trigger backlight --
       ...
       pr_emerg("Tracy_mipi_ili9486_lcd_off_cmd_start\n");
...
static void mipi_ili9486_set_backlight(struct msm_fb_data_type *mfd)
{
	...
	if (new_light_level == 0) {
		...
		gpio_set_value(GPIO_BKL_EN, 0);
		return;
	}else {
		if (!one_wire_mode_active) {
			tps_enter_one_wire_mode();
			one_wire_mode_active = 1;
		}
		udelay(100);
		tps_cmd_write(new_light_level);
		udelay(100);
	}

}
void tps_enter_one_wire_mode(void)
{
	mdelay(50);
	gpio_set_value(GPIO_BKL_EN, 1);
	udelay(100); /* t_es_delay(100us) */
	gpio_set_value(GPIO_BKL_EN, 0);
	udelay(260); /* t_es_detect(260us) */
	gpio_set_value(GPIO_BKL_EN, 1);
}

void tps_bit_set(bool bit)
{
	if (bit) {
		gpio_set_value(GPIO_BKL_EN, 0);
		udelay(2);
...
void tps_cmd_write(int level)
{
	...
	gpio_set_value(GPIO_BKL_EN, 0);
	udelay(2);
	ack=gpio_get_value(GPIO_BKL_EN);
	if(ack==0){
		gpio_set_value(GPIO_BKL_EN, 1);
		}else{
		for (i=0; i<8; i++) {
...
static struct msm_fb_panel_data ili9486_panel_data = {
	...
	.vreg_control = mipi_ili9486_reg_control,    //tracy 20121024 fix sleep current
